...
export default function QuickActions() {
  const actions = [
    ...
    { icon: Clock, label: 'History', color: 'bg-blue-500', path: '/bookings' },
  ]

  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6 sm:mb-8 px-1 sm:px-0">
      {actions.map(({ icon: Icon, label, color, badge, path }) => (
        <Link
          key={label}
          to={path}
        ...
        >
          ...
          >
            ...
            {badge && (
              <span
                className="absolute -top-1.5 -right-1.5 h-5 w-5 bg-red-500 text-white text-xs 
